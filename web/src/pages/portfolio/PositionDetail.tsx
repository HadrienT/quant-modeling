...
export default function PositionDetail({ position, onClose, onSave }: Props) {
	...
	const [category, setCategory] = useState<ProductCategoryKey>(
		position.category as ProductCategoryKey,
	);
	const [productType, setProductType] = useState<ProductTypeKey>(
		position.product_type as ProductTypeKey,
	);
	...
	return (
		<div className="pos-detail-overlay" onClick={onClose}>
			<div
				className="pos-detail-panel card"
				...
				<div className="pos-detail-header">
					<div>
						...
						<span className="pos-detail-subtitle">
							{findProduct(position.product_type as ProductTypeKey)?.label ??
								...
								<label className="field" key={p.key}>
									...
									{p.type === "select" ? (
										...
										<select
											...
											onChange={(e) =>
												setParam(p.key, e.target.value === "true")
											}
										>
						...
						<div className="pos-detail-section">
							...
							<div className="pos-detail-grid">
								{Object.entries(position.parameters).map(([key, value]) => {
									const paramDef = productDef?.params.find(
										(p) => p.key === key,
									);
									return (
										<div className="pos-detail-item" key={key}>
											<span className="pos-detail-key">
												{paramDef?.label ?? key}
											</span>
								...
								<div className="pos-detail-section">
									...
									<div className="pos-detail-grid">
										<div className="pos-detail-item">
											...
											<span className={r.npv >= 0 ? "profit" : "loss"}>
												{fmtCurrency(r.npv)}
											</span>
										</div>
										<div className="pos-detail-item">
										...
										<div className="pos-detail-item">
											...
											<span>
												{r.priced_at
													? new Date(r.priced_at).toLocaleString()
													: "—"}
											</span>
										</div>
						...
						{!r && (
							<div className="pos-detail-section">
								<p className="empty-msg">
									Not yet priced. Click "Price All" to compute.
								</p>
							</div>
						)}
					</div>
