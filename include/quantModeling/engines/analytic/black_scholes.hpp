...
public:
  ...
  void visit(const VanillaOption &opt) override {
    validate(opt);
    const auto &m =
        require_model<IBlackScholesModel>("BSEuroVanillaAnalyticEngine");
    ...
    if (type == OptionType::Call) {
      ...
      out.greeks.vega =
          opt.notional * (S0 * df_q * nd1 * std::sqrt(T)); // ∂V/∂σ
      // Theta and rho are included as common desk outputs; sign conventions
      // vary. Here: theta = ∂V/∂t (calendar time), negative typically; rho =
      // ∂V/∂r. Using standard continuous-carry formulas:
      out.greeks.rho = opt.notional * (T * K * df_r * norm_cdf(d2)); // ∂V/∂r
      out.greeks.theta =
          opt.notional * (-(S0 * df_q * nd1 * v) / (2.0 * std::sqrt(T)) -
      ...
      out.greeks.delta =
          opt.notional * (df_q * (norm_cdf(d1) - 1.0)); // call delta - df_q
